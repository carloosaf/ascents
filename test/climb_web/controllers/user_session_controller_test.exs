defmodule ClimbWeb.UserSessionControllerTest do
  ...
  describe "GET /users/log-in" do
    ...
    test "renders login page with email filled in (sudo mode)", %{conn: conn, user: user} do
      ...
      magic_email_input =
        ...
        |> LazyHTML.query("input[name='user[email]'][value='#{user.email}']")

      assert Enum.any?(magic_email_input)
    end
  ...
  describe "POST /users/log-in - email and password" do
    ...
    test "logs the user in with remember me", %{conn: conn, user: user} do
      ...
      assert conn.resp_cookies["_climb_web_user_remember_me"]
      assert redirected_to(conn) == ~p"/"
    end
  ...
  describe "POST /users/log-in - magic link" do
    test "sends magic link email when user exists", %{conn: conn, user: user} do
      ...
      assert Climb.Repo.get_by!(Accounts.UserToken, user_id: user.id).context == "login"
    end
